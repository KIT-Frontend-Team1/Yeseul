...
function inventoryControl(product, count) {
  ...
  let selectedProduct = data[product]; // 품목이 있는지 개수로 확인
  // console.log(selectedProduct) // 디버깅

  // early return
  // 3.전산표에 없는 물건이라면 확인을 위해
  // 해당 물건의 이름을 콘솔에 로그 합니다.
  if (!selectedProduct) return console.log(product);

  // 4.만약 표의 값과 갯수가 일치하지 않는다면
  // “전산표와 일치하지 않습니다”를 로그 합니다.
...
inventoryControl("바나나우유", 6); // 전산표와 일치하지 않습니다.
inventoryControl("야채곱창", 5); // 전산표와 일치합니다.
inventoryControl("바사삭치킨", 6); // 바사삭치킨
